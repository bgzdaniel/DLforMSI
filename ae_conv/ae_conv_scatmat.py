...
# guassian mixture
batch = torch.from_numpy(data[0:pixel_count//2, :]).to(device)
batch2 = torch.from_numpy(data[pixel_count//2:, :]).to(device)
encoded = None
with torch.no_grad():
    ...
    batch2 = F.pad(batch2, (0, (padded_count-intensity_count)), "constant", 0)
    batch2 = torch.unsqueeze(batch2, 1)
    encoded2 = model.module.encode(batch2).cpu().numpy()
    encoded = np.concatenate((encoded, encoded2), axis=0)
k = 6
